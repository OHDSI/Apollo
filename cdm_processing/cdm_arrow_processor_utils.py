...
def get_cdm_tables(cdm_folder: str, partition_i: int) -> Dict[str, pa.Table]:
    """
    Get all CDM tables from a folder
    """
...
def remove_concepts(combined_table: pa.Table, concept_ids: List[int]) -> tuple[pa.Table, int]:
    """
    Removes all rows from a table that have a concept ID in the given list.

    Args:
        combined_table: A table with a person_id, concept_id and a start_date column.
    ...
    """
    count_before = len(combined_table)
    result = combined_table.filter(~pc.is_in(pc.field(CONCEPT_ID), pa.array(concept_ids)))
    return result, count_before - len(result)


def remove_duplicates(combined_table: pa.Table) -> tuple[pa.Table, int]:
    """
    ...
    sql = "SELECT DISTINCT * FROM combined_table"
    result = con.execute(sql).arrow()
    duckdb.close(con)
    return result, count_before - len(result)


def link_events_to_visits(combined_table: pa.Table,
    ...
    sql = "CREATE TABLE joined_1 AS " \
          ...
          "  ON combined_table.visit_occurrence_id = visit_occurrence.visit_occurrence_id "
    con.execute(sql)
    ...
    sql = "SELECT SUM(mapped_by_id) AS mapped_by_id, " \
          "  SUM(mapped_by_date) AS mapped_by_date, " \
          "  SUM(mapped_by_new_visit) AS mapped_to_new_visit, " \
          "  COUNT(*) AS total " \
          "FROM ( " \
    ...
    duckdb.close(con)
    return combined_table, visit_occurrence, statistics
