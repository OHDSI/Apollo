...
class BertFineTuningLearningObjective(LearningObjective):
    ...
    def process_row(self, row: Dict, start_index: int, end_index: int, max_sequence_length: int) -> tuple[Dict, Dict]:
        output_dict = {ModelInputNames.LABEL: row[DataNames.LABEL]}
        return {}, output_dict
...
class VisitPredictionLearningObjective(LearningObjective):
    ...
    def process_row(self, row: Dict, start_index: int, end_index: int, max_sequence_length: int) -> tuple[Dict, Dict]:
        ...
        masked_visit_token_ids, output_mask = self._mask_visit_concepts(visit_token_ids, max_sequence_length)
        ...
        input_dict = {
            ModelInputNames.MASKED_VISIT_CONCEPTS: masked_visit_token_ids,
            ModelInputNames.MASK_VISIT: visit_mask
        }
        output_dict = {ModelInputNames.VISIT_PREDICTIONS: combined_label}
        return input_dict, output_dict

    def _mask_visit_concepts(self, visit_concepts: np.ndarray, max_sequence_length: int):
